...
if (!window.$RefreshReg$ || !window.$RefreshSig$ || !window.$RefreshRuntime$) {
  ...
  window.$RefreshReg$ = (type, id) => {
    window.$RefreshRuntime$.register(type, '"app\\\\context\\\\AuthContext.tsx"' + id);
  };
...
if (import.meta) {
  ...
  import.meta.hot.lastModified = "1750110911604.987";
}
...
function AuthProvider({
  ...
  (0, import_react.useEffect)(() => {
    const initializeAuth = () => {
      ...
      const userData = api.get("user");
      const userType = api.get("user_type");
      if (token && userData) {
        try {
  ...
  const login = async (email, password) => {
    try {
      ...
      api.set("access_token", userData.token, {
        expires: 7
      });
      api.set("user", JSON.stringify(userData), {
        expires: 7
      });
      api.set("user_type", userData.role, {
        expires: 7
      });
      setUser(userData);
      setIsAuthenticated(true);
  ...
  const register = async (data) => {
    try {
      const response = await axios_default.post("http://127.0.0.1:8000/api/signup", data);
      if (response.data) {
  ...
  return /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)(AuthContext.Provider, { value: {
    user,
    company,
    loading,
    login,
...
_s(AuthProvider, "U3sVlqmIPiYmKQtJLgTdUPLT4Xk=", false, function() {
...
*/
//# sourceMappingURL=/build/_shared/chunk-5HQ5EFJ4.js.map
